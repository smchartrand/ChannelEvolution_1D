function [ taucrit,taucritless,R2GSeval,ycrit,Scrit ] = Kirchner1990v3( R2GS,theta,exposure,z,zo,R3GSiet,VRandom )
...
% integral in Eq. 12.
    ...
    % Life coefficient - dimensionless (source Wiberg and Smith, 1985)
    Cl = 0.20;
    ...
    for i = 1:VRandom
        ...
        if exposurem(i) == 0 || evaluate(i) <= 0
            ...
            Scrit(i) = (taucritless(i) .* ( rhos - rhow) .* g .* R2GSm(i)) ./ (rhow .* g .* ycrit(i));
